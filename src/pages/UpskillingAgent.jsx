...
// Core employee data - everything derives from this
const employeeData = {
  Sales: [
...
// INTERCONNECTED DATA GENERATORS - Everything derives from employeeData
const useInterconnectedData = (selectedDept) => {
  return useMemo(() => {
    const deptEmployees = employeeData[selectedDept];
    
    // 1. SKILL GAP ANALYSIS - Generated from actual gaps
    const skillGaps = {};
    deptEmployees.forEach(emp => {
      const skillCategory = emp.gap.split(' ')[0]; // First word of gap
    ...
    // Calculate skill radar data from actual employee gaps
    const radarSkills = Object.entries(skillGaps)
      .slice(0, 5) // Top 5 skill categories
      .map(([skill, data]) => {
        const avgCurrent = Math.round(
          data.employees.reduce((sum, emp) => sum + emp.before, 0) / data.employees.length
        );
        const avgRequired = Math.min(95, avgCurrent + 25); // Required is current + improvement target
    ...
    // 2. DEMAND FORECAST - Based on actual skill gaps and impact levels
    const demandForecast = Object.entries(skillGaps)
      .slice(0, 3) // Top 3 in-demand skills
      .map(([skill, data]) => {
        ...
        const growthRate = Math.round(demandScore * 0.4); // Growth correlates with demand
    ...
    // 3. KPI CALCULATIONS - All interconnected
    ...
    // 4. HIGH PERFORMERS - UPDATED: Limited to top 5 with max score of 100
    const highPerformers = deptEmployees
      .filter(emp => emp.score >= 80)
      .map(emp => ({
        name: emp.name,
        department: selectedDept,
        productivity_score: Math.min(100, Math.round(emp.score * 1.1)), // Cap at 100
    ...
    // 5. SCATTER PLOT DATA - Performance vs Completion
    ...
    // 7. DEPARTMENT INSIGHTS - All calculated from real data
    ...
    // 8. AI INSIGHTS - Enhanced business logic
...
const UpskillingAgent = () => {
  ...
  // Color function for scatter plot
  ...
  return (
    <div className="talent-container fade-in">
      <header className="dashboard-header">
        <div className="header-content">
          <div className="header-main slide-up">
            ...
            <p className="header-subtitle">Your strategic learning architect - Accelerating talent transformation through AI-driven skill development</p>
          </div>
      ...
      <div className="main-layout">
        <div className="content-area">
          ...
          <section className="charts-section">
            ...
            <div className="charts-grid">
              ...
              {/* FIXED: Skill Score vs Task Completion Rate Scatter Plot */}
              ...
              {/* HIGH PERFORMER CHART - Top 5 employees, Y-axis max 100 */}
                    ...
                    <BarChart data={highPerformers}>
                      ...
                      <Bar dataKey="productivity_score" radius={[4, 4, 0, 0]}>
                        {highPerformers.map((entry, index) => {
                          const isTopPerformer = index < 5; // Highlight top 2
                          return (
                            <Cell 
                              key={`cell-${index}`} 
              ...
              {/* Demand Forecast - Generated from actual gaps */}
        ...
        {/* AI Sidebar - All interconnected insights */}
      ...
      {/* Table Section - Shows source employee data */}
      <section className="table-section">
        <h2 className="section-title slide-up">📋 Skills Development Tracking</h2>
        
        <div className="table-container fade-in-table">
          <div className="table-wrapper">
            <h3 className="table-title">Current Skills Development - {selectedDept} Department ({deptEmployees.length} employees)</h3>
